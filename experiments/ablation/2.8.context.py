...
dataset = sys.argv[1] if len(sys.argv) > 1 else 'liar2'
...
worksapce = workspace_map[dataset]

model_name = '2.8.context'
model_save = model_name + '.pt'

suffix = '' if dataset == 'liar2' else f'_{dataset}'
sys.stdout = open(model_name + suffix + '.out', 'a', buffering=1)
print(f'===== SEED {seed} =====')
...
class LiarDataset(data.Dataset):
    def __init__(self, data_df, statement, label_onehot, label, date, subject, speaker, speaker_description, state_info,
        ...
        self.metadata_text = torch.cat((date.int(), subject.int(), speaker.int(), speaker_description.int(), state_info.int()), dim=-1)
...
class LiarModel(nn.Module):
    ...
    def forward(self, text, metadata_text, metadata_number, justification):
        ...
        fused_output = self.fuse(torch.cat((text_output, metadata_output_text, metadata_output_number, metadata_output_number_fuzzy, justification_output), dim=1))
